...
import MatchGameBody from '../MatchGameBody'
...
class MatchGame extends Component {
  ...
  selectImageFunc = id => {
    const {imgToSelect} = this.state
    if (id === imgToSelect.id) {
      console.log('selected')
      this.setState(prevState => ({
        score: prevState.score + 1,
        imgToSelect: imagesList[Math.floor(Math.random() * imagesList.length)],
      }))
    } else {
      clearInterval(this.timerId)
      this.setState(prevState => ({inResult: !prevState.inResult}))
    }
  }
  ...
  render() {
    const {imgToSelect, selectedTab, score, seconds, inResult} = this.state

    const filteredThumbnails = imagesList.filter(
      obj => obj.category === selectedTab,
    )
    ...
    return (
      <div className="bg-container">
        ...
        <div className="main-body">
          {inResult ? (
            <GameResult score={score} playAgainFunc={this.playAgainFunc} />
          ) : (
            <MatchGameBody
              gameDetails={{imgToSelect, selectedTab}}
              filteredThumbnails={filteredThumbnails}
              selectTabFunc={this.selectTabFunc}
              selectImageFunc={this.selectImageFunc}
            />
          )}
        </div>
      </div>
